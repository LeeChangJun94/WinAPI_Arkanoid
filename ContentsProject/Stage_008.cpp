...
void AStage_008::BeginPlay()
{
	...
	LoadBrick("Stage8", PlayerLifeActor);
}

void AStage_008::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_007");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_009");
	}
}

void AStage_008::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
