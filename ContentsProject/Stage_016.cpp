...
void AStage_016::BeginPlay()
{
	...
	LoadBrick("Stage16", PlayerLifeActor);
}

void AStage_016::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_015");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_017");
	}
}

void AStage_016::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
