...
void AStage_011::BeginPlay()
{
	...
	LoadBrick("Stage11", PlayerLifeActor);
}

void AStage_011::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_010");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_012");
	}
}

void AStage_011::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
