...
void AStage_020::BeginPlay()
{
	...
	LoadBrick("Stage20", PlayerLifeActor);
}

void AStage_020::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_019");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_021");
	}
}

void AStage_020::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
