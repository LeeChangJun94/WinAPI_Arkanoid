...
void AStage_015::BeginPlay()
{
	...
	LoadBrick("Stage15", PlayerLifeActor);
}

void AStage_015::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_014");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_016");
	}
}

void AStage_015::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
