...
void AStage_030::BeginPlay()
{
	...
	LoadBrick("Stage30", PlayerLifeActor);
}

void AStage_030::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_029");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_031");
	}
}

void AStage_030::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
