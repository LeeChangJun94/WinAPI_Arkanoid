...
void AStage_012::BeginPlay()
{
	...
	Stage1->SetPlayMapType(EPlayMapType::TYPE_4);
	...
	LoadBrick("Stage12", PlayerLifeActor);
}

void AStage_012::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_011");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_013");
	}
}

void AStage_012::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
