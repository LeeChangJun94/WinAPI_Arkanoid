...
void AStage_019::BeginPlay()
{
	...
	LoadBrick("Stage19", PlayerLifeActor);
}

void AStage_019::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_018");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_020");
	}
}

void AStage_019::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
