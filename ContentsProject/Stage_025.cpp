...
void AStage_025::BeginPlay()
{
	...
	LoadBrick("Stage25", PlayerLifeActor);
}

void AStage_025::Tick(float _DeltaTime)
{
	Super::Tick(_DeltaTime);

	if (true == UEngineInput::GetInst().IsDown('N'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_024");
	}

	if (true == UEngineInput::GetInst().IsDown('M'))
	{
		UEngineAPICore::GetCore()->OpenLevel("Stage_026");
	}
}

void AStage_025::LoadBrick(std::string _Stage, APlayerLife* _PlayerLifeActor)
{
